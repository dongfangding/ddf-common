...
public class DateUtils {
    ...
    public static Long getMonthFirstSecondsByDayFormatter(Integer month) {
        return LocalDate.parse(month + "01", DateUtils.DAY_INTEGER_FORMATTER)
    ...
    public static Long getDayLastSecondsByDayFormatter(Integer monthDay) {
        return LocalDate.parse(String.valueOf(monthDay), DateUtils.DAY_INTEGER_FORMATTER).atTime(LocalTime.MAX).atZone(
                ZoneOffset.ofHours(8)).toInstant().getEpochSecond();
    }
    ...
    public static Integer formatYearMonthDayByMillis(Long milli) {
        return Integer.parseInt(
                DAY_INTEGER_FORMATTER.format(LocalDateTime.ofInstant(Instant.ofEpochMilli(milli), ZoneId.of("+8"))));
    }
