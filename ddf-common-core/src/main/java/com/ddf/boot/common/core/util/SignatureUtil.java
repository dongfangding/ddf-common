...
public class SignatureUtil {
     ...
     * <p>
     * 1. 参数名按ASCII升序排序；
     * 2. 参数值非空按 {@code k=v&k1=v2} 形式组合数据。 如果flatten为true的话，针对复杂嵌套对象，会将参数平铺成一级结构，如{@code {"a":1,"b":{"c":2,"d":3}}  => a=1&b.c=2&b.d=3}
     ...
     * 即使拿最简单的查询字符串来说，其实类似于与这样{@code param1={"id":1,"name":"haha"}&param2={"id":1,"name":"haha"}}
     * 那么其实要保证的是参数的value里面的json要有序， 否则客户端和服务端可能因为id和name的前后顺序不一致而导致加签结果不同。
     * <p>
     * 这里和{@code param1=1&param2=chen&param3=上海}    这种情况并不一致，这种查询串直接map保持一定规则就行，而不是上面那种复杂形势。
     * <p>
