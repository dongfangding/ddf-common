...
(
  ...
  COLLATE = utf8mb4_general_ci COMMENT = '通道传输报文日志记录'
  ROW_FORMAT = Dynamic;
...
(
  ...
  CHARACTER SET = utf8mb4
  COLLATE = utf8mb4_general_ci COMMENT = '设备基础管理表'
  ROW_FORMAT = Dynamic;
...
(
  ...
  CHARACTER SET = utf8mb4
  COLLATE = utf8mb4_general_ci COMMENT = '云闪付收款到账消息'
  ROW_FORMAT = Dynamic;
...
INSERT INTO `platform_template_config`(`id`, `template_type`, `template_remark`, `required_related_channel`,
                                       `default_client_channel`)
VALUES ('1', 1, '个人码到账消息', 1, NULL),
       ('2', 2, '商户码到账通知', 1, NULL),
       ('3', 3, '转账通知', 1, NULL),
       ('4', 4, '登录验证码', 1, NULL),
       ('5', 5, '转账验证码', 1, NULL),
...
insert into platform_message_template(id, title, template_type, client_channel, sort, credit, template_context)
VALUES

-- 银行到账短信模板
(1, '光大银行到账短信模板-1', 7, NULL, 2, '95595',
 '尊敬的客户：您尾号${bankCardNo}账户${hour}:${minute}存入${amount}元，余额${balance}元，摘要:${payType}—付款方姓名:${payName},付款方账号后四位:${payNo}。[${payBankName}]'),
(2, '广发银行到账短信模板-1', 7, NULL, 2, '95508',
 '【${bankName}】您尾号${bankCardNo}卡${day}日${hour}:${minute}收入人民币${amount}元（${payType}-${payName}）。'),
(222, '广发银行到账短信模板-2', 7, NULL, 2, '95508',
 '【${bankName}】您尾号${bankCardNo}卡${day}日${hour}:${minute}收入人民币${amount}元(${payType})。'),

(3, '上海浦东发展银行到账短信模板-1', 7, NULL, 2, '95528',
 '您尾号${bankCardNo}卡人民币活期${hour}:${minute}存入${amount}[${payType}:${payName}]，可用余额${balance}。【${bankName}】'),
(4, '中信银行到账短信模板-1', 7, NULL, 2, '9555801',
 '【${bankName}】您尾号${bankCardNo}的中信卡于${month}月${day}日${hour}:${minute}，${payType}存入人民币${amount}元，当前余额为人民币${balance}元。'),
(5, '民生银行到账短信模板-1', 7, NULL, 2, '95568',
 '账户${bankCardNo}于${month}月${day}日${hour}:${minute}存入￥${amount}元，可用余额${balance}元。${payType}。【${bankName}】'),
(6, '平安银行到账短信模板-1', 7, NULL, 2, '106927995511',
 '您尾号${bankCardNo}的账户于${month}月${day}日${hour}:${minuteContactPayType}转入人民币${amount}元。【${bankName}】'),
(7, '平安银行到账短信模板-2', 7, NULL, 2, '106927995511',
 '您存款账户${bankCardNo}于${month}月${day}日${hour}:${minuteContactPayType}转入人民币${amount}元，详见 pingan.com/foMI【${bankName}】'),
(8, '招商银行到账短信模板-1', 7, NULL, 2, '95555',
 '您账户${bankCardNo}于${month}月${day}日${hour}:${minuteContactPayType}（${payName}），人民币${amount}元[${bankName}]'),
(32, '招商银行到账短信模板-2', 7, NULL, 2, '95555',
 '您账户${bankCardNo}于${month}月${day}日${hour}:${minute}收到${payType}人民币${amount}，付方${payName}，账号尾号${payNo}，备注：转账[${bankName}]'),
(9, '华夏银行到账短信模板-1', 7, NULL, 2, '95577',
 '您的账户${bankCardNo}于${month}月${day}日${hour}:${minute}收入人民币${amount}元，余额${balance}元。${payType}，付款方${payName}。【${bankName}】'),
(999, '华夏银行到账短信模板-2', 7, NULL, 2, '95577',
 '${bankCardNo}于${month}月${day}日${hour}:${minute}收入人民币${amount}元，余额${balance}元。${payType}。【${bankName}】'),
(10, '中国邮政到账短信模板-1', 7, NULL, 2, '95580',
 '【${bankName}】${year}年${month}月${day}日${hour}:${minute}您尾号${bankCardNo}账户${payType}金额${amount}元，余额${balance}元。'),
(11, '工商银行到账短信模板-1', 7, NULL, 2, '95588',
 '您尾号${bankCardNo}卡${month}月${day}日${hour}:${minuteContactPayType})${amount}元，余额${balance}元。【${bankName}】'),
(12, '交通银行到账短信模板-1', 7, NULL, 2, '95559',
 '您尾号${bankCardNo}的卡于${month}月${day}日${hour}:${minuteContactPayType}转入${amount}元,交易后余额为${balance}元。【${bankName}】'),
(13, '建设银行到账短信模板-1', 7, NULL, 2, '95533',
 '${payNameContactMonth}月${day}日${hour}时${minute}分向您尾号${bankCardNo}的储蓄卡账户${payType}收入人民币${amount}元,活期余额${balance}元。[${bankName}]'),
(14, '建设银行到账短信模板-1', 7, NULL, 2, '95533',
 '您尾号${bankCardNo}的储蓄卡账户${month}月${day}日${hour}时${minute}分${payType}收入人民币${amount}元,活期余额${balance}元。[${bankName}]'),

-- 银行支付短信模板
(15, '光大银行支出短信模板-1', 8, NULL, 2, '95595',
 '尾号${bankCardNo}账户${hour}:${minute}支出${amount}元，余额${balance}元，摘要:${payType} 二维码快速收款码专用。[${bankName}]'),
(16, '广发银行支出短信模板-1', 8, NULL, 2, '95508',
 '【${bankName}】您尾号${bankCardNo}卡${day}日${hour}:${minute}支出人民币${amount}元(${payType})。'),
(17, '上海浦东发展银行支出短信模板-1', 5, NULL, 2, '95528',
 '您尾号${bankCardNo}卡${hour}:${minute}消费${amount}[${payType}],可用余额${balance}【${bankName}】'),
(18, '中信银行支出短信模板-1', 8, NULL, 2, '9555801',
 '【${bankName}】您尾号${bankCardNo}的中信卡于${month}月${day}日${hour}:${minute}，${payType}人民币${amount}元，当前余额为人民币${balance}元。'),
(19, '民生银行支出短信模板-1', 8, NULL, 2, '95568',
 '账户${bankCardNo}于${month}月${day}日${hour}:${minute}支出￥${amount}元，可用余额${balance}元。${payType}。【${bankName}】'),
(20, '平安银行支出短信模板-1', 8, NULL, 2, '106927995511',
 '您尾号${bankCardNo}的账户于${month}月${day}日${hour}:${minuteContactPayType}转出人民币${amount}元。【${bankName}】'),
(21, '招商银行支出短信模板-1', 8, NULL, 2, '95555',
 '您账户${bankCardNo}于${month}月${day}日${hour}:${minute}向${payName}做${payType}，人民币${amount}元[${bankName}]'),
(210, '招商银行支出短信模板-2', 8, NULL, 2, '95555',
 '您账户${bankCardNo}于${month}月${day}日${hour}:${minute}银联扣款人民币${amount}元（${payType}）[${bankName}]'),


(22, '华夏银行支出短信模板-1', 8, NULL, 2, '95577',
 '您的账户${bankCardNo}于${month}月${day}日${hour}:${minute}支出人民币${amount}元，余额${balance}元。${payType}。【${bankName}】'),
(23, '中国邮政支出短信模板-1', 8, NULL, 2, '95580',
 '【${bankName}】${year}年${month}月${day}日${hour}:${minute}您尾号${bankCardNo}账户消费金额${amount}元，余额${balance}元。'),
(24, '工商银行支出短信模板-1', 8, NULL, 2, '95588',
 '您尾号${bankCardNo}卡${month}月${day}日${hour}:${minute}工商银行支出(${payType})${amount}元，余额${balance}元。【${bankName}】'),
(25, '交通银行支出短信模板-1', 8, NULL, 2, '95559',
 '您尾号${bankCardNo}的卡于${month}月${day}日${hour}:${minute}网络支付异地消费${amount}元,交易后余额为${balance}元。【${bankName}】'),
(26, '交通银行支出短信模板-1', 8, NULL, 2, '95559',
 '您尾号${bankCardNo}的卡于${month}月${day}日${hour}:${minute}网络支付转出${amount}元,交易后余额为${balance}元。【${bankName}】'),
(27, '建设银行支出短信模板-1', 8, NULL, 2, '95533',
 '您尾号${bankCardNo}的储蓄卡账户${month}月${day}日${hour}时${minute}分向${payType}支出人民币${amount}元,活期余额${balance}元。[建设银行]'),

-- 垃圾短信模板
(28, '垃圾短信模板-1', 9, NULL, 999, null, '{"condition": "NOT_CONTAINS", "value": "[\"银行\"]"}'),

-- 云闪付到账消息模板-1
(29, '云闪付个人码到账消息模板', 1, 'UPAY', 1, null,
 '${payName}通过扫码向您付款${amount}元,您的收款卡尾号为${payBankName}(尾号${bankCardNo})'),
(34, '云闪付商户码到账消息模板-1', 2, 'UPAY', 0, null, '云闪付收款${amount}元。'),
(35, '云闪付转账消息模板', 3, 'UPAY', 0, null,
 '您向${receiverName}、${bankName}、${bankCardNo}转账${amount}元，已于${year}-${month}-${day} ${hour}:${minute}成功到账！'),

-- 云闪付登录验证码
(31, '云闪付登录验证码短信模板-1', 4, 'UPAY', 1, '95516',
 '验证码${verifyCode}，有效期5分钟。您正在进行账户安全验证，请勿将短信验证码告知他人！【中国银联】'),

-- 云闪付转账验证码
(33, '云闪付转账验证码短信模板-1', 5, 'UPAY', 0, '95516',
 '手机验证码为：${verifyCode}，您正在进行转账交易，验证码1分钟内有效，泄露验证码会影响资金安全。【中国银联】'),

-- SIM卡安全认证短信
(36, '安全认证短信模板-1', 10, NULL, 998, null,
 '手机验证码为：${verifyCode}，您正在进行转账交易，验证码1分钟内有效，泄露验证码会影响资金安全。【中国银联】'),

-- 忽略处理消息模板
(36666666, '忽略无用云闪付消息模板-1', 6, NULL, 998, null,
 '您尾号为${bankCardNo}的银行卡于${day}日${hour}时${minute}分消费${amount}元'),

-- 忽略处理消息模板
('20200221134801', '云闪付风控自动回复短信模板-1', 6, NULL, 2, '95516',
 '【$/{title}】尾号$/{bankCardNo}银行卡于$/{orderTimeStr}转账$/{amount}元。为保障您的资金安全，请立即回复“$/{verifyCode}”确认交易，如不是本人交易，请立即联系发卡行挂失并拨打$/{bankTel}。（20分钟内回复有效）【中国银联】'),

-- 注册验证码
(202004261609001, '建行app注册验证码-1', 12, 'CCB_APP', 999, '95533',
 '验证码${verifyCode}，有效期5分钟。您正在注册云闪付账号，请勿泄露！如非您本人操作，请忽略。【中国银联】'),

-- 建行转账验证码
(202004261658001, '建行app转账验证码-1', 5, 'CCB_APP', 999, '95533',
 '序号${ignore}的验证码${verifyCode}，您向${targetAccountName}尾号${targetAccountNo}账户转账${amount}元。任何索要验证码的都是骗子，千万别给！[建设银行]');
