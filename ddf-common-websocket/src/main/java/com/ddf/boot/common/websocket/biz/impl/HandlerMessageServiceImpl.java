...
public class HandlerMessageServiceImpl implements HandlerMessageService {
    ...
    public void handlerMessage(AuthPrincipal authPrincipal, WebSocketSessionWrapper webSocketSessionWrapper, TextMessage textMessage) {
        if (AuthPrincipal.LoginType.ANDROID.equals(authPrincipal.getLoginType())) {
            doAndroidMessageConsumer(authPrincipal, webSocketSessionWrapper, textMessage);
        }
    }
     ...
     * @param textMessage
     */
    private void doAndroidMessageConsumer(AuthPrincipal authPrincipal, WebSocketSessionWrapper webSocketSessionWrapper, TextMessage textMessage) {
        handlerMessagePool.execute(() -> {
            if (CmdEnum.PING.name().equals(textMessage.getPayload())) {
                pingCmdStrategy.responseCmd(authPrincipal, textMessage);
                return;
            }
