...
if (maxValue ~= nil and result > maxValue) then
    -- 超出最大限制将值设置允许的最大值
    redis.call('hset', KEYS[1], hashKey, maxValue)
    -- actualStep代表实际递增的值
    return cjson.encode({ limited = 1, actualStep = maxValue - (result - step), currentValue = maxValue })
end
-- 首次的话设置过期时间
if (ARGV[5] ~= "" and tonumber(ARGV[5]) > 0 and redis.call('HEXISTS', KEYS[1], hashKey) == 0) then
