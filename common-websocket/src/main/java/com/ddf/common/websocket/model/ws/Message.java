...
import com.ddf.common.util.AESUtil;
import com.ddf.common.util.JsonUtil;
import com.ddf.common.util.MD5Util;
...
public class Message<T> {
    ...
    public static TextMessage wrapperWithSign(@NotNull Message message) {
        ...
        String sign = MD5Util.encodeSalt(body, message.getCmd().name());
        message.addExtra("sign", sign);
        return new TextMessage(AESUtil.encrypt(JsonUtil.asString(message)));
    }
    ...
    public static Message unSign(String textMessagePayload) {
        ...
        String decrypt = AESUtil.decrypt(textMessagePayload);
        ...
        String dataSign = MD5Util.encodeSalt(JsonUtil.asString(message.getBody()), message.getCmd().name());
