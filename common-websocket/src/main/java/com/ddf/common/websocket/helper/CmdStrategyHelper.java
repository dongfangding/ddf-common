...
import com.ddf.common.websocket.exception.InvalidStatusException;
import com.ddf.common.websocket.exception.MessageFormatInvalid;
import com.ddf.common.websocket.exception.MessageTemplateNotFoundException;
import com.ddf.common.websocket.exception.MessageTemplateParseException;
...
public class CmdStrategyHelper {
    ...
    private void parseSmsDetail(AuthPrincipal authPrincipal, MerchantBaseDevice baseDevice, Message message
        ...
        handlerMatchTemplateExecutor.execute(() -> {
            ...
            if (!parseContent.isGarbage() && !Objects.equals(matchTemplate.getCredit(), smsContent.getCredit())) {
                log.error("短信发送方未通过认证.短信中发件人: {}, 模板中认证标识: {}, ！！！{}",
                        smsContent.getCredit(), matchTemplate.getCredit(), merchantMessageInfo);
                merchantMessageInfo.setStatus(MerchantMessageInfo.STATUS_LOGIC_ERROR);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void doUPayMessageBusiness(AuthPrincipal authPrincipal, Message message, List<MerchantMessageInfo> infos) {
        handlerMessageBusiness.execute(() -> {
            if (infos == null || infos.isEmpty()) {
                return;
            }
            MerchantBaseDevice baseDevice = checkDevice(authPrincipal, infos);

            for (MerchantMessageInfo messageInfo : infos) {
                parseUPayMessageDetail(authPrincipal, message.getCmd(), baseDevice, messageInfo);
            }
        });
    }

    /**
     * 详细解析云闪付到账消息步骤(单独写是因为重试会需要，但是重试的数据不止一个设备的，重试那里因为数据多，可以
     * 提前把所有的设备一个sql查出来)
     *
     * @param authPrincipal
     * @param cmd
     ...
     */
    private void parseUPayMessageDetail(AuthPrincipal authPrincipal, CmdEnum cmd, MerchantBaseDevice baseDevice
            , MerchantMessageInfo messageInfo) {
        handlerMatchTemplateExecutor.execute(() -> {
            UPayMessage uPayMessage;
            String errorMessage;
            messageInfo.setMerchantId(baseDevice.getMerchantId());
            messageInfo.setDeviceId(baseDevice.getId());
            try {
                uPayMessage = JsonUtil.toBean(messageInfo.getSingleMessagePayload(), UPayMessage.class);
                if (uPayMessage == null) {
                    throw new MessageFormatInvalid("云闪付到账消息转换后内容为空!");
                }
                messageInfo.setDescription(uPayMessage.getContent());
                if (StringUtils.isAnyBlank(uPayMessage.getContent(), uPayMessage.getOrderId())) {
                    throw new MessageFormatInvalid("云闪付到账内容、订单号不能为空!");
                }
                if (uPayMessage.getOrderTime() == null) {
                    throw new MessageFormatInvalid("订单时间不能为空！");
                }
            } catch (Exception e) {
                log.error("云闪付消息内容解析失败！！！内容: {}", messageInfo.getSingleMessagePayload(), e);
                errorMessage = StringUtil.exceptionToString(e);
                messageInfo.setStatus(MerchantMessageInfo.STATUS_DATA_INVALID);
                messageInfo.setErrorMessage(errorMessage);
                messageInfo.setErrorStack(StringUtil.exceptionToString(e));
                merchantMessageInfoService.fillStatus(messageInfo, baseDevice);
                return;
            }
            List<PlatformMessageTemplate> templates = platformMessageTemplateService.getUPayMessageTemplate();

            ParseContent parseContent;
            try {
                if (templates == null || templates.isEmpty()) {
                    throw new MessageTemplateNotFoundException("未配置云闪付到账消息模板!");
                }
                parseContent = parseMessageTemplate(templates, uPayMessage.getContent());
                if (parseContent == null) {
                    throw new MessageTemplateParseException("云闪付到账消息匹配有误！请检查模板！");
                }
                if (parseContent.getAmount() == null || uPayMessage.getOrderTime() == null) {
                    throw new MessageFormatInvalid("金额和订单时间都不能为空!");
                }
                parseContent.byUPayMessage(uPayMessage, cmd);
            } catch (MessageTemplateNotFoundException e) {
                log.error("异常消息: {} => 到账数据：{}", e.getMessage(), messageInfo);
                messageInfo.setStatus(MerchantMessageInfo.STATUS_NOT_MATCH_TEMPLATE);
                messageInfo.setErrorMessage(e.getMessage());
                merchantMessageInfoService.fillStatus(messageInfo, baseDevice);
                return;
            } catch (MessageTemplateParseException e) {
                log.error("云闪付到账消息匹配有误！请检查模板！[{}]", uPayMessage.getContent());
                messageInfo.setStatus(MerchantMessageInfo.STATUS_NOT_MATCH_TEMPLATE);
                messageInfo.setErrorMessage(e.getMessage());
                merchantMessageInfoService.fillStatus(messageInfo, baseDevice);
                return;
            } catch (MessageFormatInvalid e) {
                log.error("解析后报文数据有误！！[{}]", uPayMessage.getContent());
                messageInfo.setStatus(MerchantMessageInfo.STATUS_DATA_INVALID);
                messageInfo.setErrorMessage(e.getMessage());
                merchantMessageInfoService.fillStatus(messageInfo, baseDevice);
                return;
            ...
            // 匹配订单
            handlerMatchOrder.handler(authPrincipal, parseContent, baseDevice, messageInfo);
        });
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void doUPayBillOrderBusiness(AuthPrincipal authPrincipal, Message message,
            List<MerchantMessageInfo> infoList) {
        handlerMessageBusiness.execute(() -> {
            MerchantBaseDevice baseDevice = checkDevice(authPrincipal, infoList);
            if (baseDevice == null) {
                return;
            ...
            for (MerchantMessageInfo merchantMessageInfo : infoList) {
                parseUPayBillOrderDetail(authPrincipal, message.getCmd(), baseDevice, merchantMessageInfo);
            }
        });
    }
     ...
     */
    private void parseUPayBillOrderDetail(AuthPrincipal authPrincipal, CmdEnum cmd, MerchantBaseDevice baseDevice
        ...
        handlerMatchTemplateExecutor.execute(() -> {
            ...
            singleMessagePayload = merchantMessageInfo.getSingleMessagePayload();
            try {
                uPayBill = JsonUtil.toBean(singleMessagePayload, UPayBill.class);
                if (uPayBill == null) {
                    throw new MessageFormatInvalid("云闪付账单数据解析后为空,请检查报文格式！");
                }
                if (StringUtils.isBlank(uPayBill.getTradeNo())) {
                    throw new MessageFormatInvalid("订单号不能为空");
                }
                orderTime = checkTimeFormat(uPayBill.getOrderTime());
                if (uPayBill.getAmount() == null) {
                    throw new MessageFormatInvalid("金额不能为空!");
                }
                // 为了兼容历史接口，为空也算成功
                if (UPayBill.STATUS_FAILURE.equals(uPayBill.getStatus())) {
                    throw new InvalidStatusException("无效的数据状态，不能作为有效数据！");
                }
            } catch (Exception e) {
                log.error("账单内容解析失败！！！内容: {}", singleMessagePayload, e);
                errorMessage = StringUtil.exceptionToString(e);
                merchantMessageInfo.setStatus(MerchantMessageInfo.STATUS_DATA_INVALID);
                merchantMessageInfo.setErrorMessage(errorMessage);
                merchantMessageInfo.setErrorStack(StringUtil.exceptionToString(e));
            ...
            ParseContent parseContent = ParseContent.byUPayBill(uPayBill, cmd).setOrderTime(orderTime);
            handlerMatchOrder.handler(authPrincipal, parseContent, baseDevice, merchantMessageInfo);
        });
    }
    ...
    public void retryMerchantMessageInfo() {
        ...
        for (MerchantMessageInfo info : retryInfos) {
            ...
            } else if (CmdEnum.UPAY_MESSAGE.name().equals(info.getCmd())) {
                parseUPayMessageDetail(authPrincipal, CmdEnum.UPAY_MESSAGE, baseDevice, info);
            } else if (CmdEnum.UPAY_BILL_ORDER.name().equals(info.getCmd())) {
                parseUPayBillOrderDetail(authPrincipal, CmdEnum.UPAY_BILL_ORDER, baseDevice, info);
            }
        }
    }
}
